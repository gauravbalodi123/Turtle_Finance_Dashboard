...
router.post('/addAdvisor', async (req, res) => {
    try {
        ...
        sendClientUpsert(newAdvisor, 'created', 'Advisors')
            .catch(err => console.error('[webhook] advisor.created failed:', err?.message));

        res.status(200).json({ msg: "Advisor has been added successfully" });
    } catch (e) {
        res.status(400).json({ msg: "Oops, something went wrong while creating an advisor" });
    }
});
...
router.patch('/advisors/:id/editAdvisors', async (req, res) => {
    try {
        ...
        sendClientUpsert(updatedAdvisor, 'updated', 'Advisors')
            .catch(err => console.error('[webhook] advisor.updated failed:', err?.message));
        ...
        let response = {
            msg: "Oops, something went wrong while editing the data for an advisor"
        };
...
router.delete('/advisors/:id/', async (req, res) => {
    ...
    if (!deleteAdvisor) {
        return res.status(404).json({ msg: "The Advisor does not exist or cannot be fetched" });
    }

   
    sendClientDelete(id, 'Advisors')
        .catch(err => console.error('[webhook] advisor.deleted failed:', err?.message));
