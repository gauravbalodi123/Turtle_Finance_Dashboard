            ...
            const matchingRows = rows
                ...
                .map(row => {
                    ...
                    return {
                        ...
                        leadSource: properties["Lead Source"]?.rich_text?.[0]?.text?.content || "",
                        ...
                        comments: properties["Comments / Reviews"]?.rich_text?.[0]?.text?.content
                            ? [{ text: properties["Comments / Reviews"]?.rich_text?.[0]?.text?.content }]
                            : [],
                        subscriptionDate: properties["Subscription Date"]?.date?.start ? new Date(properties["Subscription Date"].date.start) : null,
                        onboardingStatus: properties["Onboarding Status"]?.select?.name || "",
                        riskProfileDate: properties["Risk Profile Date"]?.date?.start ? new Date(properties["Risk Profile Date"].date.start) : null,
                        kickOffDate: properties["Kick Off Date"]?.date?.start ? new Date(properties["Kick Off Date"].date.start) : null,
