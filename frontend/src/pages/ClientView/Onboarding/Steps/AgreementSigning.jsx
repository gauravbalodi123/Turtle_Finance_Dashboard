...
const AgreementSigning = ({ onNext, onPrev }) => {
    ...
    const checkDigioResponse = async () => {
      try {
        ...
        if (response.status === 200) {
          setDigioValid(true);
          setErrorSign(null);
          onNext(); // ✅ already signed → skip ahead
        }
      } catch (error) {
        if (error.response) {
  ...
  const handlePrepareContract = async (e) => {
    e.preventDefault();
    console.log("🟢 [handlePrepareContract] User clicked Generate Agreement");
    ...
    if (!digioLoaded) {
      console.error("⚠️ Digio SDK not loaded yet");
    ...
    try {
      console.log("🟡 Preparing Digio options...");
      const digioOptions = {
        environment: digioEnvironment,
        callback: async (digioResponse) => {
          console.log("📩 Digio callback triggered:", digioResponse);

          if (digioResponse.hasOwnProperty("error_code")) {
            console.error("❌ Digio signing error:", digioResponse);
            setErrorSign("Error during signing.");
          } else {
            try {
              console.log("📡 Sending Digio response to backend...");
              await axios.post(`${url}/client/save-digio-response`, digioResponse, {
                headers: {
                  Authorization: `Bearer ${localStorage.getItem("token")}`,
                },
              });
              console.log("✅ Digio response saved successfully");
              setSuccessSign(true);
              onNext();
            } catch (err) {
              console.error("❌ Failed to save Digio response:", err);
      ...
      console.log("📡 Calling backend:", `${url}/client/upload-document`);
      ...
      if (response.data) {
        ...
        digio.init();
        console.log("🟢 Digio SDK initialized");

        const identifier = response.data.signing_parties?.[0]?.identifier;
        const digioDocumentId = response.data.id;
        console.log("📄 Digio Document ID:", digioDocumentId);
        console.log("👤 Signing party identifier:", identifier);

        digio.submit(digioDocumentId, identifier);
        console.log("🚀 Digio submit triggered");
      }
    } catch (err) {
      console.error("❌ Error in handlePrepareContract:", err);
      setErrorSign(err.response?.data?.error || "An error occurred while generating the contract.");
    } finally {
      setLoadingSign(false);
      console.log("🔵 Finished handlePrepareContract");
    }
  };
